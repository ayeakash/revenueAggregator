#!/usr/bin/env python3
"""
YouTube KPI Poster — Views ONLY (Colab & CI Ready)

- Pulls per-channel views from YouTube Analytics.
- Posts one Slack message (Views) with totals and per-channel lines.
...
SHEET_TAB          = os.getenv("SHEET_TAB", "facts_views")
...
def info_banner():
    print("="*60)
    print("YouTube KPI Poster — Views ONLY")
    print("="*60)
...
def write_facts_to_sheets(per_chan, sheet_url, sheet_tab):
    if not sheet_url:
    ...
    for label, d in per_chan.items():
        if d.get("views_latest") is None:
            continue
        facts.append({
            "as_of_day": d["views_latest"].isoformat(),
            "channel": label,
    ...
    cols = ["as_of_day", "channel", "y_views", "mtd_views", "last_month_views"]
    for c in cols:
...
def build_views_sections(labels, per_chan):
    y_lines = []
    proj_lines = []
    tot_y = 0.0
    tot_mtd = 0.0
    ...
    for label in labels:
        ...
        y_val = d.get("y_views")
        mtd_val = d.get("mtd_views")
        last_val = d.get("last_month_views")
        ...
        if latest is None or mtd_val is None:
            ...
            proj_lines.append(f"• {label}: {fmt_number(ch_proj)}{pct_change(ch_proj, last_val)}")
            tot_mtd += mtd_val
...
def slack_self_test():
    ...
    try:
        r = requests.post(SLACK_WEBHOOK_URL, json={"text": "✅ Slack webhook test: Hello from YouTube Views bot."}, timeout=10)
...
def main():
    ...
    try:
        if GOOGLE_SHEET_URL:
            write_facts_to_sheets(per_chan, GOOGLE_SHEET_URL, SHEET_TAB)
        else:
            print("[Sheets] Skipped (GOOGLE_SHEET_URL empty)")
    except Exception as e:
    ...
    # Slack
    slack_self_test()
    blocks = build_views_sections(labels_order, per_chan)
    ...
    for label in labels_order:
        ...
        print(f"{label}: Views(Y/MTD/LM)={vY}/{vM}/{vL}")
